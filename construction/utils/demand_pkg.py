...
def get_back_timespan(df, sku_dc_pair, train_date, minus, period):
    ...
    #             pd.date_range(start=x['train_date']-timedelta(days=(minus+30)), periods=(period-x['len_common'])), axis=1 )
    # print ('4')

    # return df_key.apply(lambda x: pd.Series(df.loc[x.item_sku_id, x['date_diff'].append(x['date_common']) ].values \
...
def get_forward_timespan(df, sku_dc_pair, train_date, period):
    ...
    # df_key['date_diff'] = df_key.apply(lambda x: \
    #                     pd.date_range(start=x['train_date']+timedelta(days=(minus+30)), 
    #                                   periods=(period))[-(period-len_common):] 
    #                     if period > x['len_common'] else pd.to_datetime([]), 
    #                     axis=1 )

    # return df_key.apply(lambda x: pd.Series(df.loc[x.item_sku_id, x['date_common'].append(x['date_diff']) ].values \
    #                         if x.item_sku_id in df.index else [np.nan]*period  
    #                         ), axis=1)
