...
class data_parser:
    ...
    def add_target(self, X, path, filename, path_to, file_to_save):
        ...
        X['demand_RV_list'] = X.apply(lambda x: df_sl.loc[x['item_sku_id'], \
                                            x['create_tm'].date():x['next_complete_dt'].date()].values\
                            if x['item_sku_id'] in df_sl.index else [], axis=1)

        X['demand_RV'] = X.apply(lambda x: sum(x['demand_RV_list']), axis=1)
        X['demand_RV_dp'] = X.apply(lambda x: sum(x['demand_RV_list']) if len(x['demand_RV_list']) <= 9 
                                                else sum(x['demand_RV_list'][:-1]), axis=1)
        ...
        X['target_decision'] = X['demand_RV_dp'] - X['initial_stock']
        X['target_decision'].clip(0.0, inplace=True)

        X['target_decision_nobc'] = X['demand_RV_dp'] - X['demand_V'] \
...
dummy_cut(output_path, feature_file4, 'df_e2e.pkl')
